...
weekdays = pd.DataFrame(
                data=get_weekday_result(matches.iloc[0]["matchday"]),
                columns=[
                    "utc_date",
                    "status",
                    "time",
...
weekday = matches.iloc[0]["matchday"]
st.header(f"Week {weekday} Results")
st.dataframe(weekdays, use_container_width=True)
